...
<span style="color: red;">由语法可知：一个类主要由属性和方法组成。</span>
...
<span style="color: red;">注意：每个对象都是相互独立的，在内存中占有独立的内存地址，并且每个对象都具有自己的生命周期，当一个对象的生命周期结束时，对象就变成了垃圾，由 Java 虚拟机自带的垃圾回收机制处理。</span>
