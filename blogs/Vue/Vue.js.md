...
<span style="color: red;">在开发环境下不要使用压缩版本，不然你就失去了所有常见错误相关的警告!</span>
...
<span style="color: red;">PS:先引用vue的js文件.PS2:每个 Vue 应用都需要通过实例化 Vue 来实现。</span>
...
<span style="color: red;">CSS属性名必须用驼峰命名法</span>
...
<span style="color: red;">或者把属性名和索引也迭代出来:</span>
...
多选时 (绑定到一个数组)：
<span style="color: red;">多选时，用Ctrl+鼠标左键 选中选项。</span>
...
> <span style="color: red;">方式①：直接把 JS 代码写在 v-on 指令中</span>
...
> <span style="color: red;">方式②：把被调用方法名写在v-on指令中，在methods对象中定义具体方法</span>
...
> <span style="color: red;">方式③：在 v-on 指令中也可以用 JS 语句中调用方法：</span>
...
<span style="color: red;">这段代码的功能是把messgae的字符进行逆序输出.
例如：message="Hello"，则打印输出"olleH"</span>

<span style="color: blue;">如果表达式存在过多逻辑，html页面就会变得臃肿不堪，难以维护。为了简单逻辑，我们可以使用计算属性。计算属性可以完成各种复杂的逻辑，包括运算、函数调用等，只要最终返回一个结果就可以。并使html页面看起来美观，简洁。</span>
...
<span style="color: red;">vue.js中计算属性默认只有 getter，因为是默认值所以我们也常常省略不写，如下代码：</span>
...
<span style="color: red;">本例中计算属性的值为fullName</span>
...
<span style="color: red;">可以使用 methods 来替代 computed，效果上两个都是一样的</span>
...
<span style="color: red;">当watch监听的数据属性发生改变时。会触发watch区域的方法。</span>
...
<span style="color: blue;">组件的data属性必须是一个函数。只有这样当组件复用时。每个组件才能改变自己的data属性。若不是函数。则改变某一个组件的data属性值时。其他复用组件的data属性值都会改变。</span>，
...
prop 是父组件用来传递数据给子组件的一个自定义属性。<span style="color: red;">在组件中定义prop属性就相当在该组件的data中添加属性。</span>

**父组件的数据需要通过 props 把数据传给子组件，子组件需要显式地用 props 选项声明 "prop"**


<span style="color: blue;">注意: prop 是单向绑定的：当父组件的属性变化时，将传导给子组件，但是不会反过来。</span>
...
<span style="color: red;">在vue对象或组件的整个生命周期中，都有对应的函数来表示对象或组件的当前状态。每当组件或对象状态进行变化时，会触发这些函数。</span>
...
<span style="color: red;">注意：浏览器本身不能识别less/sass等文件。webpack就是把这些文件进行打包，编译，变成浏览器能识别的js,html文件</span>
...
<span style="color: red;">使用命令 vue -V , 查看是否安装成功。</span>
