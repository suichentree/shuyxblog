...
<span style="color: red;">Add python.exe to PATH 是把python添加到环境变量中。</span>
...
<span style="color: red;">例如，对字符串的一个索引位置赋值，比如str[0] = 'm'会导致错误。</span>
...
<span style="color: red;">元组即元素的组合。</span>

<span style="color: red;">元组是写在方括号( )之间、用逗号分隔开的不能修改元素列表。简而言之元组就是一旦初始化就不能修改的列表。</span>
...
<span style="color: red;">注意：创建一个空集合必须用 set()构造函数，而不是 {}。因为 {} 是用来创建一个空字典</span>
