...
# 博客页面部署的仓库  https://gitee.com/suichentree/suichentree.git   main分支 这个远程仓库的仓库别名是 gitee_page_origin ，是默认仓库
# 博客页面部署的仓库  https://github.com/suichentree/suichentree.github.io.git  main分支 这个远程仓库的仓库别名是 github_page_origin ，非默认仓库
...
# 定义pushBlogPage方法
function pushBlogPage(){
    # 本地分支推送文件到远程仓库origin的main分支, -f 强制提交
    git push gitee_page_origin main -f
    ...
    git push github_page_origin main -f
    # $?可以获取git push 命令是否运行成功，成功返回0，否则非0。
    if [ $? -eq 0 ] 
    then
        # 上传成功，方法结束
...
# 初始化 dist文件夹 为git仓库,默认分支为main分支
git init -b main
...
git remote add gitee_page_origin https://gitee.com/suichentree/suichentree.git
git remote add github_page_origin https://github.com/suichentree/suichentree.github.io.git
